...
Также в связи с большой площадью расположения клиентов, для уменьшения времени отдачи врмени на отдачу статических файлов имеет смысл использовать CDN, для кэширования этих файлов уже в регионе пользователя, либо ближайщем к нему региону с CDN сервером
...
* **Сетевой трафик**
**Пиковая нагрузка** 1,26 * 0,25 = 0,315 Гбит/с
...
* Nginx
Nginx позволяет поддерживать 4,830 rps HTTPS запросов размером с 100 Кб на одном ядре [[8]](#source-4). С учётом пиковой нагрузки в 0,19 Гбит/с и числом запросов в 835,45, получаем что размер одного запроса что покрывает по сути нагрузку на весь сервис. Для обеспечения такой производительности, возьмём конфигурацию из [[8]](#source-4).
...
Пересчитаем значение метрик под такую нагрузку:
* **Сетевой трафик**
**Пиковая нагрузка** 1,26 * 0,1 = 0,126 Гбит/с
...
Суммарный RPS равен 3 341,809 * 0,1 = ***334,181***

...
2. **Поиск**
В среднем, человек обычно ищет какую-то определенную вещь, также возможно, его не устраивает результат запроса и он отправляет уточненный запрос, то есть осуществляет ***2*** запроса по поиску + ***~5*** страниц с объявлениями за каждый поиск.
...
2. **Поиск**
   12 * 13 710 000 / 24 / 60 / 60 = ***1 902,36***
...
Для репликации данных между разными датацентрами используется расшерение BDR для PostgreSQL, позволяющее делать master-master репликацию. Репликация происходит на основе логической репликации. Данное расшерение, предоставляет функционал для решения большинства конфликтов при применении даннх на репликах.
Но для решения конфликта о нарушении внешнего ключа, при удалении записи объявления имеет смысл создать поле, которое бы говорила о том. что запись удалена. Тем самым существующие транзакции бронирования, которые могли произойти в момент удаления таблицы, просто выполняться без конфликтов внешних ключей.
А дальнейшие попытки создать бронирования будут останавливаться на этапе проверки доступности объявления. После чего сервер пометивший запись как удалённую спустя некоторое время её удаляет.

Для сессий тоже используется репликация master-master в Redis. Данна репликация реализуется инструментом Redis Cluster. Для решения проблемы возможных задержак по синхранизации данных между серверами, можно добавить к сессионному айди некоторый индентификатор сервера, который её выдал и, когда происходит запрос клиента на какой либо сервер, который не находит в своём хранилище запись пользователя, этот сервер бы обратился к серверу, который создал эту запись, для проверки наличия её нём.
...
Расчитаем конфигурацию серверов на датацентре согласно самому нагруженному, т.е. 20% от нагрузки, с запасом будет 25%.
...
Сумарный объём данных составляет около ~100 Гб, возьмём размер памяти с запасом.

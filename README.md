...
* Nginx
Nginx позволяет поддерживать 4,830 rps HTTPS запросов размером с 100 Кб на одном ядре [[8]](#source-4), что покрывает по сути нагрузку на весь сервис. Для обеспечения такой производительности, возьмём конфигурацию из [[8]](#source-4).
| **Параметр** | **Значение** |
| :-: | :-: |
| CPU (cores) | 1 |
| Network GbE | 40 |
